...
export function initWorkspace() {
  ...
  const updateFields = () => body.querySelectorAll('[data-field]').forEach((input) => input.addEventListener('change', () => { state[input.dataset.field] = input.value; persist(); if (input.dataset.field === 'character') render() }))
  ...
  function renderGuided() {
    ...
    resetStep.addEventListener('click',()=>{state[key]=defaults[key];persist();render()})
  ...
  function renderShake() {
    const keys = ['age','product','theme','character','artStyle','fashion','typography','palette','material','composition','production']
    body.innerHTML = `<p class="mode-intro">A controlled creative shuffle. Lock what you love, then shake the rest.</p><div class="shake-grid">${keys.map((key) => `<button type="button" class="shake-choice ${locks.has(key)?'is-locked':''}" data-lock="${key}"><small>${key.replace(/([A-Z])/g,' $1').toUpperCase()}</small><strong>${escapeHtml(state[key])}</strong><span>${locks.has(key)?'🔒 LOCKED':'TAP TO LOCK'}</span></button>`).join('')}</div><div class="builder-actions"><button data-shake>ϟ SHAKE AGAIN</button><button data-lock-all>🔒 LOCK THIS</button><button data-build>BUILD PROMPT</button></div>`
  ...
  function formMarkup(extra = '') {
    const characterFields = `${field('character','CHARACTER',state.character)}${isMascotCharacter(state.character) ? field('mascot','ORIGINAL MASCOT TYPE',state.mascot) : ''}${isHumanCharacter(state.character) ? field('hairstyle','HAIRSTYLE',state.hairstyle) : ''}`
