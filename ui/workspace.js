...
export function initWorkspace() {
  ...
  function renderGuided() {
    ...
    const suppressed = (['ethnicity','hairstyle'].includes(key) && !isHumanCharacter(state.character)) || (key === 'expression' && state.character === 'No character')
  ...
  function formMarkup(extra = '', special = false) {
    const characterFields = `${field('character','CHARACTER',state.character)}${isMascotCharacter(state.character) ? field('mascot','ORIGINAL MASCOT TYPE',state.mascot) : ''}${isHumanCharacter(state.character) ? `${field('ethnicity','ETHNICITY / CULTURAL BACKGROUND',state.ethnicity)}${state.ethnicity === 'Custom' ? field('customCulturalBackground','CUSTOM CULTURAL BACKGROUND',state.customCulturalBackground,null) : ''}${field('hairstyle','HAIRSTYLE',state.hairstyle)}` : ''}${state.character !== 'No character' ? field('expression','EXPRESSION',state.expression) : ''}`
    const themeField = special ? field('occasionTheme','CELEBRATION THEME',state.occasionTheme,options.occasionTheme) : field('theme','THEME',state.theme)
    return `<div class="form-grid">${field('age','AGE GROUP',state.age)}${field('product','PRODUCT',state.product)}${themeField}${field('mood','DESIGN MOOD',state.mood)}${characterFields}${field('pose','POSE / ACTION',state.pose)}${field('artStyle','ART STYLE',state.artStyle)}${field('fashion','FASHION',state.fashion)}${field('typography','TYPOGRAPHY',state.typography)}${field('phrase','EXACT PHRASE (OPTIONAL — AUTO-MATCHED IF BLANK)',state.phrase,null)}${field('palette','COLORS',state.palette)}${field('material','FAUX MATERIAL',state.material)}${field('composition','COMPOSITION',state.composition)}${field('production','PRINT METHOD',state.production)}${field('intensity','CREATIVE INTENSITY',state.intensity)}${extra}</div>`
  }
  ...
  function renderModeForm() {
    ...
    body.innerHTML = `<p class="mode-intro">${mode === 'Special Occasions' ? 'Build an original birthday, holiday, cultural, seasonal, school, or family-celebration design. Only celebration-specific themes appear here; selected traditions stay distinct, respectful, and age-appropriate.' : 'Set the shared creative direction. The engine will preserve coordination while forcing meaningful variation.'}</p>${formMarkup(extra,mode === 'Special Occasions')}<div class="builder-actions"><button data-build>BUILD PROMPT</button></div>`
    updateFields()
