...
export function buildOutfit(state) {
  ...
  const items = parts.map((part, index) => { const settings={ ...state, product:part, composition:index ? 'Coordinated supporting composition' : state.composition }; return {...composePrompt(settings,{ placement, coordination:`${distribution[index] || `supporting visual moment ${index + 1}`}; distribute the outfit's visual information instead of overloading one garment` }),title:`${state.outfit} — ${part}`,settings} })
