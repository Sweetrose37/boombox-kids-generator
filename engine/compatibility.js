...
export function resolveCompatibility(input) {
  ...
  const notes = []
  if (selections.production !== 'DTF') {
    notes.push({ field:'production', from:selections.production, to:'DTF', reason:'all generated artwork is required to be DTF production ready' })
    selections.production = 'DTF'
  }
  ...
  if (selections.composition === 'All-over composition') {
