...
export function buildMatchMini(state) {
  const first = composePrompt(state, { phraseIndex:0, coordination: `${state.relationship}; share theme, palette family, typography family, motif, and material direction while keeping each look distinct` })
  ...
  const second = composePrompt(alternate, { phraseIndex:1, coordination: `companion look for ${state.relationship}; vary garment, phrase, pose, character styling, composition, and placement without assuming gender, ethnicity, or biological relationship` })
