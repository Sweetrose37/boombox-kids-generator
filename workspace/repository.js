...
export class WorkspaceRepository {
  ...
  createPrompt(output,state={},mode='Build with BooBoo',extra={}) {
    const prompt = normalizePrompt({ title:output.title, designConcept:output.direction, prompt:output.prompt, age:output.age, product:output.product, production:output.production,
      intensity:state.intensity, artStyle:state.artStyle, character:state.character, mascot:state.mascot, ethnicity:state.ethnicity, customCulturalBackground:state.customCulturalBackground, specialOccasion:mode==='Special Occasions'?state.specialOccasion:'', birthdayAge:mode==='Special Occasions'?state.birthdayAge:'', customOccasion:mode==='Special Occasions'?state.customOccasion:'', typography:state.typography, palette:state.palette, material:state.material, exactPhrase:output.exactPhrase || state.phrase,
