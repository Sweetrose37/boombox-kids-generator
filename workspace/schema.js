...
export function normalizePrompt(raw={}) {
  ...
  return {
    ...
    intensity:text(raw.intensity,'PLAYFUL'), artStyle:text(raw.artStyle), character:text(raw.character), mascot:text(raw.mascot), ethnicity:text(raw.ethnicity,'Not specified'), customCulturalBackground:text(raw.customCulturalBackground), expression:text(raw.expression), specialOccasion:text(raw.specialOccasion), occasionTheme:text(raw.occasionTheme), birthdayAge:text(raw.birthdayAge), customOccasion:text(raw.customOccasion), typography:text(raw.typography), palette:text(raw.palette), material:text(raw.material),
